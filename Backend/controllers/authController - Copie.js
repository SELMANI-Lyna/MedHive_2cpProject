...
// Fonction utilitaire pour valider le format de l'email (basique)
const isValidEmailFormat = (email) => {
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return emailRegex.test(email);
};

// Fonction utilitaire pour vérifier si l'email existe réellement
const verifyEmailExists = async (email) => {
    try {
        const result = await validate({
            ...
            validateSMTP: false // Mettre à true pour une validation SMTP plus stricte (mais plus lent)
        });
        ...
        // En cas d'erreur de validation, on fait une vérification basique
        return {
            isValid: isValidEmailFormat(email),
            reason: 'Impossible de vérifier l\'existence de l\'email',
            validators: {}
        };
    }
};
...
exports.inscrirePharmacien = async(req, res) => {
    try {
        ...
        const emailValidation = await verifyEmailExists(email);
        if (!emailValidation.isValid) {
            return res.status(400).json({
                success: false,
                message: `L'adresse email n'existe pas ou n'est pas valide. ${emailValidation.reason}`,
                details: emailValidation.validators
            });
        }
        ...
        const token = jwt.sign({ id: nouvelUtilisateur._id, role: 'Pharmacien' },
...
exports.inscrireVendeur = async(req, res) => {
    try {
        const { email, motDePasse, telephone, username, photoProfil, facebook, instagram, linkedin } = req.body;
        ...
        const token = jwt.sign({ id: nouvelUtilisateur._id, role: 'Vendeur' },
            process.env.JWT_SECRET, { expiresIn: '24h' }
        );
...
exports.connexion = async(req, res) => {
    try {
        ...
        // Validation du format de l'email uniquement pour la connexion (pas besoin de vérifier l'existence)
...
exports.motDePasseOublie = async(req, res) => {
    try {
        ...
        // Validation du format de l'email uniquement (pas besoin de vérifier l'existence pour reset)
        if (!isValidEmailFormat(email)) {
            return res.status(400).json({
                success: false,
                message: "Format d'email invalide."
...
exports.verifierCode = async(req, res) => {
    try {
        ...
        // Validation du format de l'email uniquement
        if (!isValidEmailFormat(email)) {
            return res.status(400).json({
                success: false,
                message: "Format d'email invalide."
